...
static void * xscore_port_seq_start(struct seq_file *m, loff_t *pos);
static void * xscore_port_seq_next(struct seq_file *m, void *data, loff_t *pos);
static void   xscore_port_seq_stop(struct seq_file *m, void *data);
static int xscore_port_seq_show(struct seq_file *m, void *data);
...
static const struct seq_operations xscore_ports_seq_ops = {
		.start		= xscore_port_seq_start,
		.next		= xscore_port_seq_next,
		.stop		= xscore_port_seq_stop,
		.show		= xscore_port_seq_show,
};
static ssize_t xscore_port_proc_write_device(struct file *file, const char __user *buffer,
                                 size_t count, loff_t *offp)
{
...
static void *xscore_port_seq_start(struct seq_file *p, loff_t *pos) {
	printk(KERN_INFO "Inside xscore_port_seq_start.\n");
	return NULL + (*pos == 0);
}

static void *xscore_port_seq_next(struct seq_file *p, void *v, loff_t *pos) {
	printk(KERN_INFO "Inside xscore_port_seq_next.\n");
	++*pos;
	return NULL;
}

static void xscore_port_seq_stop(struct seq_file *p, void *v) {
	printk(KERN_INFO "Inside xscore_port_seq_stop.\n");
}

static int xscore_port_seq_show(struct seq_file *m, void *data)
{
	struct xscore_port_data *port_data = (struct xscore_port_data *)data;

	printk(KERN_INFO "Inside xscore_port_seq_show. %p\n", data);
	if (data == NULL) {
...
static int xscore_port_open_proc_device(struct inode *inode, struct file *file) {
	...
	res = seq_open(file, &xscore_ports_seq_ops);
	if (!res) {
		m = file->private_data;
